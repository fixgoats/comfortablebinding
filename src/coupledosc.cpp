...
int main(const int argc, const char* const* argv) {
  ...
  options.add_options()("c,config", "TOML configuration",
      ...
      "v,verbose", "Verbose output")("h, help", "show this help text");
  ...
  if (static_cast<bool>(result["h"].count()) || result.arguments().empty()) {
    std::cout << options.help() << std::endl;
    return EXIT_SUCCESS;
  }
  ...
  if (static_cast<bool>(result["c"].count())) {
    ...
    spdlog::info("Got {} confs", confs.size());
    for (const auto& conf : confs) {
      spdlog::info("Running {}", conf->type());
      conf->run();
    }
  }
}
