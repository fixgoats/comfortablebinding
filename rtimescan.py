...
ax[0,1].grid()
ax[0, 2].set_title("$|\\sum_n psi_n(t)|^2$, logscale")
ax[0, 2].set_ylabel("t")
ax[1, 2].set_title("$|ifft(\\sum_n psi_n(t))|^2$, logscale")
...
for fname in args.files:
    ...
    im2 = ax[1, 0].imshow(np.abs(np.angle(snapshot)), aspect="auto", interpolation="none", cmap="inferno", origin="lower", extent=(rs[0], rs[-1], 0, np.shape(snapshot)[1]))
    cb2 = plt.colorbar(im2)
    ax[0, 1].set_xlim(left=rs[0], right=rs[-1])
    ...
    jydelta = jymax - jymin
    ax[0,1].set_ylim(bottom=jymin-0.02*jydelta, top=jymax+0.02*jydelta)
    lines = []
    for d in unique_coupling_lengths:
        j = sp.special.jv(0, rs*d)
        y = sp.special.yv(0, rs*d)
        line1, = ax[0, 1].plot(rs, j, label=f"$J_0(rscale\\cdot {d:.3f})$")
        line2, = ax[0, 1].plot(rs, y, label=f"$Y_0(rscale\\cdot {d:.3f})$")
        lines.append(line1)
        lines.append(line2)
    ax[0,1].legend()
    ...
    im3 = ax[0,2].imshow(sqNorm(psiseries[:,::10].T), extent=(rs[0], rs[-1], t0, t1), aspect="auto", norm=LogNorm(), origin="lower", interpolation="none")
    im4 = ax[1,2].imshow(psifftsqnorm.T[1024-64:1024+64,:], extent=(rs[0], rs[-1], energies[1024-64], energies[1024+64]), origin="lower", aspect="auto", norm=LogNorm(),interpolation="none")
